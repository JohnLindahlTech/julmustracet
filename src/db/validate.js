...
function doValidation(db, newDoc, options) {
  ...
  if (
    String(newDoc._id).indexOf("_design/") === 0 ||
    String(newDoc._id).indexOf("_local") === 0
  ) {
  ...
  return getValidationFunctions(db).then(function (validationFuncs) {
    ...
    return Promise.all([completeOptionsPromise, oldDocPromise]).then(
      Function.prototype.apply.bind(function (completeOptions, oldDoc) {
...
wrapperApi.putAttachment = function (orig, args) {
  return args.base
    .get(args.docId, { rev: args.rev, revs: true })
    .catch(function () {
...
Object.keys(wrapperApi).forEach(function (name) {
  ...
  exports[exportName] = wrappers.createWrapperMethod(
    name,
    orig,
    wrapperApi[name]
  );
